...
Vector3 Spline::Compute(size_t startIdx, float t) const
{
	...
	// compute position accorting to catmull-rom equation
...
void SplineCamera::Update(float deltaTime)
{
	...
	{
		mT += mSpeed * deltaTime;
		// advance to the next control pt if needed
		// this assumes speed isn't so fast that you jump past
		// multiple control points in one frame
		if (mT >= 1.0f)
		{
			//make sure we have enough points do advance the path
	...
	//cameraposition is the spline at the curr t/index
